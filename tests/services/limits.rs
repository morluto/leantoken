...
async fn reconcile_working_tree_generation_checks_run_after_reconciliation() {
    ...
    // The cursor format is valid (4 fields) but the generation mismatch
    // causes rejection, which triggers reconciliation.
    request.cursor = Some("search:0:0000000000000000:0".into());
    let error = services
        .search_with_consistency_cancellable(
            request,
            IndexConsistency::ReconcileWorkingTree,
