...
pub(super) struct FacetPlan {
    ...
    pub(super) allow_role_diversity: bool,
}

impl FacetPlan {
    pub(super) fn mark_resolved(&mut self, fusion_key: &str) {
        for facet in &mut self.facets {
            if facet.fusion_key == fusion_key {
                facet.unresolved = false;
            }
        }
    }

    pub(super) fn unresolved(&self) -> impl Iterator<Item = &TaskFacet> {
        self.facets.iter().filter(|facet| facet.unresolved)
    }
}
...
pub(super) fn plan(task: &str, limit: usize) -> FacetPlan {
    ...
    let allow_role_diversity = facets.iter().any(|facet| {
        matches!(
            facet.kind,
            FacetKind::Behavior | FacetKind::TestIntent | FacetKind::Configuration
        )
    });
    let queries = build_queries(&facets, limit, wants_tests);
    FacetPlan {
        facets,
        queries,
        wants_tests,
        allow_role_diversity,
    }
}

fn build_queries(facets: &[TaskFacet], limit: usize, wants_tests: bool) -> Vec<ContextQuery> {
    let available = limit.saturating_sub(usize::from(wants_tests));
    let mut queries = Vec::new();
    let mut positions = HashMap::<String, usize>::new();
    let behavior_reserve = facets
        .iter()
        .filter(|facet| matches!(facet.kind, FacetKind::Behavior | FacetKind::Configuration))
        .count()
        .min(4)
        .min(available);
    let exact_facets = facets
        .iter()
        .filter(|facet| facet.kind == FacetKind::ExactAtom)
        .collect::<Vec<_>>();
    let primary_exact = exact_facets
        .iter()
        .filter(|facet| is_primary_atom(&facet.original))
        .copied()
        .collect::<Vec<_>>();
    let secondary_exact = exact_facets
        .iter()
        .filter(|facet| !is_primary_atom(&facet.original))
        .copied()
        .collect::<Vec<_>>();
    let technical_capacity = available.saturating_sub(behavior_reserve);
    let expansion_reserve = technical_capacity
        .saturating_sub(primary_exact.len())
        .min(3);
    let secondary_capacity = technical_capacity
        .saturating_sub(primary_exact.len())
        .saturating_sub(expansion_reserve);
    let exact_limit = primary_exact.len().min(technical_capacity) + secondary_capacity;

    for facet in primary_exact
        .into_iter()
        .chain(secondary_exact)
        .take(exact_limit)
    {
        push_query(
            &mut queries,
    ...
    while queries.len() < available {
        ...
        for primary in [true, false] {
            for facet in facets.iter().filter(|facet| {
                facet.kind != FacetKind::TestIntent && is_primary_atom(&facet.original) == primary
            }) {
                let Some(value) = facet.variants.get(round) else {
                    continue;
                };
                let exact_variant = facet.kind == FacetKind::ExactAtom
                    && value.eq_ignore_ascii_case(&facet.original);
                push_query(
                    &mut queries,
                    &mut positions,
                    facet,
                    value,
                    exact_variant,
                    if exact_variant { 2.0 } else { facet.weight },
                    available,
                );
...
fn push_query(
    ...
    if queries.len() >= limit || value.chars().count() < 2 {
        return;
    }
    if !exact_variant && is_stop_word(value) {
        return;
    }
    let normalized = value.to_ascii_lowercase();
    if let Some(position) = positions.get(&normalized).copied() {
        let query = &mut queries[position];
        query.weight = query.weight.max(facet.weight);
        query.concept_weight = query.concept_weight.max(concept_weight);
        query.exact_variant |= exact_variant;
...
fn technical_atoms(task: &str) -> Vec<String> {
...
fn is_primary_atom(value: &str) -> bool {
    value.starts_with("#[")
        || value.starts_with('@')
        || value.contains("::")
        || value.contains('.')
...
mod tests {
    ...
    #[test]
    fn exact_atom_is_the_first_variant_and_survives_stop_words() {
            ...
            .filter(|facet| facet.kind == FacetKind::ExactAtom)
        {
            ...
            assert!(
                plan.queries
                    .iter()
                    .any(|query| query.value == facet.original && query.exact_variant)
            );
        }
    }
    ...
    #[test]
    fn kebab_case_error_atoms_produce_a_bounded_phrase_variant() {
        let plan = plan(
        ...
        assert!(
            plan.queries
                .iter()
                .any(|query| query.value == "failed to lookup view")
        );
