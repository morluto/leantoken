use crate::receipt::{
    ...
    ReceiptEvidence, decide, format_receipt_id, parse_receipt_id,
};
    ...
    tx.query_row(
        ...
        |row| {
            Ok(PersistentReceiptRow {
                ...
                evidence_count: i64_to_usize(row.get(5)?)?,
                evidence_bytes: i64_to_usize(row.get(6)?)?,
            })
        },
    )
    .optional()
...
fn load_receipt_evidence(
    ...
    let mut statement = tx.prepare(
        "SELECT path, start_line, end_line, content_hash, semantic_signature
