impl Storage {
    ...
    /// require a consistent multi-page view must use [`GenerationReadTransaction::list_files`]
    /// on one generation transaction because file replacement can assign a new row id.
    pub fn list_files(&self, max_results: usize, cursor: Option<i64>) -> Result<Vec<FileRecord>> {
        self.begin_generation_read()?
            .list_files(max_results, cursor)
    }

    pub fn find_file(&self, path: &str) -> Result<Option<FileRecord>> {
        self.begin_generation_read()?.find_file(path)
    }
    ...
    pub fn get_chunks_for_file(
        ...
        self.begin_generation_read()?
            .get_chunks_for_file(file_id, max_results)
    }
    ...
    pub fn counts(&self) -> Result<StorageCounts> {
        self.begin_generation_read()?.counts()
    }
}
use super::*;
