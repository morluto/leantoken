...
pub(super) fn create_current_cache(
...
pub(super) fn create_scoped_cache(
    manager: &CacheManager,
    repository: &Path,
    scope: &IndexScope,
) -> (String, PathBuf) {
    ...
    let database = directory.join(DATABASE_NAME);
    drop(
        Storage::open_for_repository_scoped(&database, repository, scope.full_digest())
            .expect("scoped cache database"),
    );
    (id, database)
}

pub(super) fn create_cache_with_content_identity(
    manager: &CacheManager,
    repository: &Path,
    accessed_at: u64,
    version: Option<u32>,
...
pub(super) fn create_legacy_wal_cache(manager: &CacheManager, id: &str, accessed_at: u64) {
    let directory = manager.root.join(id);
    fs::create_dir_all(&directory).expect("cache directory");
