...
fn discovery_skill(launcher: &McpLauncher) -> Result<String> {
    ...
    Ok(format!(
        "---\nname: leantoken\ndescription: Use LeanToken for token-bounded repository exploration, audits, codebase investigations, architecture reviews, source archaeology, code search, symbol outlines, exact source reads, and symbol history.\n---\n\n{DISCOVERY_SKILL_MARKER}\n\nBefore retrieving repository source, including for audits and code archaeology, discover the deferred `leantoken` MCP server and route progressively:\n\n1. `leantoken.files` finds paths or returns a compact tree.\n2. `leantoken.outline` maps definitions and imports; `leantoken.search` locates symbols, references, identifiers, text, or regex matches.\n3. `leantoken.read` returns only the exact current symbol or narrow line range needed; `leantoken.history` reads, diffs, or traces that symbol across immutable Git revisions.\n\nUse `leantoken.context` only while scope remains uncertain; pass `BASE..HEAD` as `base_revision` with `strict_changed_paths` for immutable range context. Use `leantoken.savings` for repository-local savings. Use native workspace tools for edits, commands, tests, runtime probes, non-symbol Git operations, and evidence that is not source retrieval. If the server or tools cannot be discovered, run `{doctor}` and report its structured registration, launch, handshake, and catalog status instead of silently claiming LeanToken was used.\n"
    ))
}
...
mod tests {
    ...
    fn setup_manages_compact_discovery_skills_without_overwriting_unowned_content() {
        ...
        for effect in &report.discovery_plan {
            ...
            assert!(
                skill.find("leantoken.files").unwrap() < skill.find("leantoken.outline").unwrap()
            );
            assert!(
                skill.find("leantoken.outline").unwrap() < skill.find("leantoken.read").unwrap()
            );
