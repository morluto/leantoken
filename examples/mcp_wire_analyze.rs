...
struct ResultLifetime {
    ...
    provider_first_visible_turn: Option<u64>,
    provider_visible_turn_count: Option<u64>,
...
struct Report {
    ...
    total_source_tokens: Option<usize>,
    ...
    same_range_duplicate_ranges: usize,
    reread_ranges: usize,
...
#[derive(Debug, Default)]
struct LegacyInputAccumulator {
    events: usize,
    tokens: OptionalSum,
}

struct TraceContext {
    parsed_messages: Vec<Option<serde_json::Value>>,
    call_tools: HashMap<String, String>,
    removed_at_turn: HashMap<String, u64>,
    provider_visibility_by_result: HashMap<String, (usize, Option<u64>)>,
}

impl LegacyInputAccumulator {
    fn add(&mut self, value: Option<u64>) {
        self.tokens.add(value, self.events == 0);
        self.events += 1;
    }

    fn finish(&self) -> Option<u64> {
        self.tokens.finish(self.events)
    }
}
...
fn analyze_trace(
    ...
    let mut category_legacy_input = BTreeMap::<String, LegacyInputAccumulator>::new();
    ...
    let mut overall_legacy_input = LegacyInputAccumulator::default();
    ...
    let TraceContext {
        ...
        call_tools,
        removed_at_turn,
        provider_visibility_by_result,
    } = inspect_trace_events(&trace, tokenizer)?;

    let mut result_lifetimes = Vec::new();
    let mut exact_range_events = HashMap::<RangeKey, usize>::new();
    ...
    let mut source_tokens_complete = true;
    let mut same_range_duplicate_ranges = 0usize;
    ...
    for (event_index, (event, message)) in trace.events.iter().zip(&parsed_messages).enumerate() {
        ...
        let usage = event.provider_usage.as_ref();
        ...
        category_usage
            ...
            .or_default()
            .add(usage);
        category_legacy_input
            .entry(category)
            .or_default()
            .add(event.provider_input_tokens);
        overall_usage.add(usage);
        overall_legacy_input.add(event.provider_input_tokens);
        ...
        if !event.ranges.is_empty() {
            ...
            for range in &event.ranges {
                ...
                if let Some(previous_event) = exact_range_events.insert(exact, event_index) {
                    same_range_duplicate_ranges += 1;
                ...
                if let Some(tokens) = range.source_tokens {
                    total_source_tokens = total_source_tokens.saturating_add(tokens);
                } else {
                    source_tokens_complete = false;
                }
                ...
                if range.repository_generation < latest_generation {
                    stale_generation_ranges += 1;
                }
                latest_generation = latest_generation.max(range.repository_generation);
            }
            let result_id = event
                .result_id
                .clone()
            ...
            let compacted_through = removed_at_turn
                .get(&result_id)
                .map(|turn| turn.saturating_sub(1));
            let last_visible_turn = [
                event.visible_through_turn,
                compacted_through,
                trace.final_turn,
            ]
            .into_iter()
            ...
            let provider_visibility = event
                .provider_visible_payload
                .as_deref()
                .map(|payload| (tokenizer.count(payload), event.turn))
                .or_else(|| provider_visibility_by_result.get(&result_id).copied());
            let provider_visible_result_tokens = provider_visibility.map(|(tokens, _)| tokens);
            let provider_first_visible_turn = provider_visibility.and_then(|(_, turn)| turn);
            let provider_visible_turn_count = provider_first_visible_turn
                .zip(last_visible_turn)
                .and_then(|(first, last)| (last >= first).then_some(last - first + 1));
            let provider_visible_token_amplification = provider_visible_turn_count
            ...
            result_lifetimes.push(ResultLifetime {
                ...
                provider_first_visible_turn,
                provider_visible_turn_count,
                source_tokens,
    ...
    for (category, usage) in category_legacy_input {
        if let Some(cost) = event_categories.get_mut(&category) {
            cost.provider_input_tokens = usage.finish();
        }
    }
    ...
    let provider_total_input_tokens = trace
        .provider_total_input_tokens
        .or_else(|| overall_legacy_input.finish());
    let total_source_tokens =
        (range_identity_count > 0 && source_tokens_complete).then_some(total_source_tokens);
...
fn inspect_trace_events(
    trace: &Trace,
    tokenizer: &Tokenizer,
) -> Result<TraceContext, Box<dyn Error>> {
    let mut context = TraceContext {
        parsed_messages: Vec::with_capacity(trace.events.len()),
        call_tools: HashMap::new(),
        removed_at_turn: HashMap::new(),
        provider_visibility_by_result: HashMap::new(),
    };
    for event in &trace.events {
        ...
        {
            context.call_tools.insert(call_id, tool_name);
        }
        ...
        if let (Some(result_id), Some(payload)) =
            (&event.result_id, &event.provider_visible_payload)
        {
            context
                .provider_visibility_by_result
                .entry(result_id.clone())
                .or_insert_with(|| (tokenizer.count(payload), event.turn));
        }
        context.parsed_messages.push(message);
    }
    Ok(context)
}
...
fn report_limitations(
    ...
    if result_lifetimes.is_empty() {
        limitations.push(Limitation {
            code: "range_identity_missing",
            detail: "No result carries range identity, so source cost, rereads, changed hashes, stale generations, and retention are unobservable.",
        });
    } else if result_lifetimes.iter().any(|lifetime| {
    ...
    if !result_lifetimes.is_empty() && total_source_tokens.is_none() {
        limitations.push(Limitation {
            code: "source_tokens_partial",
            detail: "At least one range identity lacks source_tokens, so the complete source-token total remains null.",
        });
    }
    limitations
}
...
mod tests {
    ...
    fn schema_v1_is_accepted_with_explicit_legacy_limitations() {
        ...
        assert_eq!(report.provider_total_input_tokens, Some(12));
    ...
    #[test]
    fn schema_v2_rejects_content_changed_after_sealing() {
        let mut trace = test_trace(vec![event(
            Direction::ClientToServer,
            0,
            serde_json::json!({"jsonrpc": "2.0", "id": 1, "method": "tools/list"}),
        ...
        trace.seal_content_hash().expect("seal trace");
        trace.events[0].raw_json = Some("{\"tampered\":true}".into());

        let error = trace.validate_version().expect_err("tampering must fail");
    ...
    fn compaction_ends_result_visibility_before_the_compaction_turn() {
        ...
        result.result_id = Some("result-1".into());
        result.visible_through_turn = Some(5);
        result.ranges = vec![range(1, "hash-a", 10)];
    ...
    #[test]
    fn final_turn_and_handoff_payload_complete_result_lifetime() {
        let mut result = event(
            Direction::ServerToClient,
            1,
            serde_json::json!({
        ...
        let mut handoff = event(
            Direction::Handoff,
            2,
            serde_json::json!({"type": "provider_handoff"}),
        );
        handoff.result_id = Some("result-1".into());
        handoff.provider_visible_payload = Some("provider payload".into());

        let report = analyze_trace(
            test_trace(vec![result, handoff]),
            "hash".into(),
            &Tokenizer::default(),
        )
        .expect("report");
        let lifetime = &report.result_lifetimes[0];
        assert_eq!(lifetime.last_visible_turn, Some(3));
        assert_eq!(lifetime.visible_turn_count, Some(3));
        assert!(lifetime.provider_visible_result_tokens.is_some());
        assert_eq!(lifetime.provider_first_visible_turn, Some(2));
        assert_eq!(lifetime.provider_visible_turn_count, Some(2));
        assert_eq!(
            lifetime.provider_visible_token_amplification,
            lifetime
                .provider_visible_result_tokens
                .map(|tokens| u64::try_from(tokens).unwrap() * 2)
        );
    }
    ...
    fn rereads_changed_hashes_and_stale_generations_are_distinct() {
        ...
        let report = analyze_trace(test_trace(events), "hash".into(), &Tokenizer::default())
            .expect("report");
        assert_eq!(report.same_range_duplicate_ranges, 1);
    ...
    #[test]
    fn duplicate_ranges_in_one_result_are_not_counted_as_rereads() {
        let mut result = event(
            Direction::ServerToClient,
            1,
            serde_json::json!({"jsonrpc": "2.0", "id": 1, "result": {}}),
        );
        result.ranges = vec![range(1, "hash-a", 5), range(1, "hash-a", 5)];
        ...
        .expect("report");
        assert_eq!(report.same_range_duplicate_ranges, 1);
        assert_eq!(report.reread_ranges, 0);
        assert_eq!(report.total_source_tokens, Some(10));
    }

    #[test]
    fn repository_generation_staleness_applies_across_paths() {
        let mut current = event(
            Direction::ServerToClient,
            1,
            serde_json::json!({"jsonrpc": "2.0", "id": 1, "result": {}}),
        );
        current.ranges = vec![range(3, "hash-a", 5)];
        let mut stale = event(
            Direction::ServerToClient,
            2,
            serde_json::json!({"jsonrpc": "2.0", "id": 2, "result": {}}),
        );
        let mut stale_range = range(2, "hash-b", 5);
        stale_range.path = "src/main.rs".into();
        stale.ranges = vec![stale_range];

        let report = analyze_trace(
            test_trace(vec![current, stale]),
            "hash".into(),
            &Tokenizer::default(),
        )
        .expect("report");
